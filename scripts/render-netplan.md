...
The four per-node files `node/etc/<alias>/40-cx7.yaml` are **generated** from `cluster.env`.
...
scripts/render-netplan.sh --write      # write node/etc/<alias>/40-cx7.yaml for the 4 ranks
scripts/render-netplan.sh --out <dir>  # render/compare somewhere else (dry runs)
```

`--check` exits 0 only when the four files on disk match the rendered ones — comment and
blank lines ignored — and the `FABRIC_TARGETS` the plan implies equals the one in
...
| Key | Role |
| --- | --- |
| `NODES` | the four ssh aliases in rank order 0..3; the alias is the directory name under `node/etc/` |
| `FABRIC_TARGETS` | per rank, the fabric addresses of that rank's two ring peers — the only address source |
| `FABRIC_IFACES` | optional; the fabric interface names of one node in file order. The first two are the addressed ports (f0, f1), the rest are rendered MTU-only. Default: the verified ASUS Ascent GX10 names, the same list as `IFACES` in `node/etc/common/tp4-fabric-iptables.sh` |
...
* **Port mapping:** odd links (`L1`, `L3`) live on the **f0** port, even links (`L2`, `L4`) on
  ...
  live files already carry, and the one `docs/fabric.md` § IP plan documents.
