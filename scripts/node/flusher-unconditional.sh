...
# In production nothing starts it by hand: `tp4ctl up` runs it on every node as the transient
# systemd unit `tp4-flusher` (systemd-run --unit=tp4-flusher --collect) and stops it — via
# `systemctl stop tp4-flusher` plus a pkill fallback — once the engine is serving or the run
# dies. Started manually, stop it with: pkill -f flusher-unconditional.sh
set -u
