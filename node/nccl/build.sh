...
# workstation. It reproduces `~/nccl-patched/libnccl.so.2` (docs/nccl.md) starting from a
...
# nothing is distributed (that is node/nccl/install-nccl.sh's job).
#
# Usage: node/nccl/build.sh [--host <alias>] [--dest <dir>] [--jobs N] [--dry-run]
...
# node/nccl/SHA256SUMS and node/nccl/expected.env (sha, size, dynamic symbol count). The
...
tp4_load_env "$HERE/../.."          # cluster.env is optional here: only NODES is used, for the default host
...
log "--- hand-off ---"
if [ "$sha_state" = MATCH ]; then
  log "node/nccl/install-nccl.sh --from '$HOST:$DEST/nccl/build/lib/libnccl.so.2'"
else
  log "node/nccl/install-nccl.sh --from '$HOST:$DEST/nccl/build/lib/libnccl.so.2' --force"
